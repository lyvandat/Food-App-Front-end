...
let datapage_callAPI = [
  ...
  {
    img: foodThum1,
    name: "Bún Đậu Mắm Tôm không ngon lắm",
    link: "/item",
    rating: 4,
    rvcount: 12.567,
    price: 89,
...
function Posts() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        setPageCount(
          prods.length % LIST_LENGTH !== 0
            ? Math.floor(prods.length / LIST_LENGTH) + 1
            : Math.floor(prods.length / LIST_LENGTH)
        );
  ...
  const handleChangePost = (type) => {
    if (type === "all") {
      ...
      let newPageCount =
        posts.length % LIST_LENGTH !== 0
          ? Math.floor(posts.length / LIST_LENGTH) + 1
          : Math.floor(posts.length / LIST_LENGTH);
      setPageCount(newPageCount);
      changepagenumber(1, newPageCount);
    } else if (type === "1star") {
      let list_1star = [];
      datapage_callAPI.forEach((post) => {
        if (post.rating === 1) list_1star.push(post);
      });
      console.log("vcl", list_1star);
      posts = list_1star;
      setPosts(list_1star);
      let newPageCount =
        list_1star.length % LIST_LENGTH !== 0
          ? Math.floor(list_1star.length / LIST_LENGTH) + 1
          : Math.floor(list_1star.length / LIST_LENGTH);
      setPageCount(newPageCount);
      changepagenumber(1, newPageCount);
    }
  };
  const countPostByStar = (star) => {
    ...
    datapage_callAPI.map((post) => {
      if (post.rating === 1) ++number_of_post;
    });
  ...
  const changepagenumber = (index, total) => {
    if (index > 0 && index <= total) {
      ...
      if (posts.length <= end) end = posts.length;
      for (; start < end; start++) {
    ...
    <div className="admin-content pt-2">
      <div className="row bg-white border-radius-smooth">
        ...
        <div className="col-12 col-md-3">
          <div className="d-flex justify-content-center pb-2">
            ...
            >
              ...
              <div className="post-square-num">{datapage_callAPI.length}</div>
            </button>
          </div>
          <div className="d-flex justify-content-center pb-4">
            ...
            >
              ...
              <div className="post-square-num">{countPostByStar(1)}</div>
            </button>
          </div>
        </div>
        <div className="col-12 col-md-9">
          ...
          <div className="row pb-4">{createPageNumber(page_count)}</div>
        </div>
      </div>
    </div>
  );
